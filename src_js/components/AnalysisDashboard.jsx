...
export const AnalysisDashboard = ({ data }) => {
  ...
  const strengthsData = (data?.strengths || []).map((s) => ({
    name: s.name,
    score: s.score,
  }));
  const weaknessesData = (data?.weaknesses || []).map((w) => ({
    name: w.name,
    score: w.score,
  }));
  ...
  return (
    <div
      className="w-full max-w-7xl mx-auto space-y-8 pb-20 px-4 sm:px-6 font-sans"
      ref={dashboardRef}
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 organic-card bg-tech-card/90 border border-tech-border/80">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Comprehensive compatibility feedback compiled from your upload
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 self-start md:self-auto text-xs">
          ...
          <span className="text-neon-emerald bg-neon-emerald/5 border border-neon-emerald/20 px-2.5 py-1 rounded font-bold">
            COMPLETED
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <motion.div
            ...
            className={cn(
              "p-6 organic-card flex flex-col items-center justify-center text-center border relative overflow-hidden",
              getScoreBgClass(data.atsScore),
            )}
          >
            ...
            <div className="relative w-36 h-36 mb-6">
              <svg className="w-full h-full transform -rotate-90">
                ...
                <motion.circle
                  ...
                  animate={{
                    strokeDashoffset: `${264 - (264 * data.atsScore) / 100}%`,
                  }}
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span
                  ...
                  style={{
                    color: getScoreColorHex(data.atsScore),
                    textShadow: `0 0 15px ${getScoreColorHex(data.atsScore)}40`,
                  }}
                >
                  {data.atsScore}
                </span>
            ...
            <div className="space-y-3">
              {(data?.jobRoles || []).map((role, idx) => (
                ...
                >
                  ...
                  <span className="font-semibold text-slate-300 text-sm tracking-tight truncate flex-1">
                    {role}
                  </span>
                </div>
              ))}
          ...
          >
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-serif font-black text-white tracking-wide uppercase">
                    EXECUTIVE SUMMARY
                  </h4>
                  <p className="text-[10px] text-slate-400">
                    Key career compatibility insights
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-6">
                <div>
                  ...
                  <p className="text-[10px] font-mono text-slate-400">
                    Comparative chronological matrix values
                  </p>
                </div>
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-[#0c1220] p-4 rounded-xl border border-tech-border shadow-2xl font-mono text-xs text-slate-100">
                              <p className="text-neon-cyan font-bold mb-1">
                                [{item.sessionName}]
                              </p>
                              <p className="text-slate-400 text-[10px] mb-2">
                                {item.date}
                              </p>
                              <p className="text-base font-black text-white font-serif">
                                {item.score}% ATS_INDEX
                              </p>
                            </div>
                          );
                        }
                          ...
                          if (active && payload && payload.length) {
                            return (
                              <div className="bg-[#0c1220] p-3 rounded-lg border border-tech-border shadow-xl font-mono text-[11px]">
                                <p className="text-neon-emerald font-bold">
                                  {payload[0].payload.name}
                                </p>
                                <p className="text-white mt-1 font-bold">
                                  POWER INDEX: {payload[0].value}%
                                </p>
                              </div>
                            );
                          }
                          ...
                          if (active && payload && payload.length) {
                            return (
                              <div className="bg-[#0c1220] p-3 rounded-lg border border-tech-border shadow-xl text-[11px]">
                                ...
                                <p className="text-white mt-1 font-bold">
                                  CAREER BOOST FACTOR: {payload[0].value}%
                                </p>
                              </div>
                            );
                          }
          ...
          >
            ...
            <div className="space-y-4">
              {(data?.improvements || []).map((imp, i) => (
                <div
                  key={i}
                  className="p-4 bg-slate-900/60 border border-tech-border rounded-xl flex items-start gap-3 hover:border-neon-cyan/30 transition-all duration-300"
                >
                  ...
                  <span className="text-xs font-medium text-slate-300 leading-relaxed font-sans">
                    {imp}
                  </span>
                </div>
              ))}
