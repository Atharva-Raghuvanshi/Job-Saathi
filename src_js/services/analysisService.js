...
export async function analyzeResume(resumeText) {
  ...
  const missingSkills = premiumGrowthPool
    .filter(
      (skill) =>
        !foundSkills.map((s) => s.toLowerCase()).includes(skill.toLowerCase()),
    )
  ...
  if (foundSkills.length < 10)
    improvements.push(
      "Your technical keyword density is a bit low. Try to explicitly list more of the tools and technologies you've used.",
    );
  if (!text.includes("education"))
    improvements.push(
      "Ensure your educational background is clearly visible to recruiters.",
    );
  if (resumeText.length < 800)
    improvements.push(
      "Your resume is a bit brief. Try to elaborate more on your specific contributions in each role.",
    );
  if (foundSections.length < 5)
    improvements.push(
      "Standard sections like 'Summary' or 'Certifications' could help give a more complete picture of your profile.",
    );
...
export async function compareResumes(text1, text2) {
  ...
  const diff1 = analysis1.skillsFound.filter(
    (skill) => !analysis2.skillsFound.includes(skill),
  );
  const diff2 = analysis2.skillsFound.filter(
    (skill) => !analysis1.skillsFound.includes(skill),
  );
  ...
  return {
    ...
    keyDifferences: [
      ...
      winner === "tie"
        ? "Both resumes are quite competitive."
        : `Resume ${winner} shows a slightly more comprehensive skill profile.`,
    ],
